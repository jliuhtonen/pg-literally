...
describe('sql', () => {
  ...
  it('supports nested sql fragments', () => {
    ...
    assert.equal(result.text, 'SELECT * FROM (SELECT * FROM users WHERE id = $1)')
  ...
  it('support more complicated fragment query', () => {
    ...
    assert.equal(normalizeWhitespace(query.text), `SELECT * FROM albums JOIN artists ON albums.artist_id = artists.id WHERE id = $1 AND title NOT IN ($2, $3) GROUP BY artist_id ORDER BY $4 LIMIT $5`)
    assert.deepEqual(query.values, [1, 'Richard D. James album', 'Syro', 'year', 5])
  })
